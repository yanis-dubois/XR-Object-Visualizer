...
namespace SMBLibrary.Client
{
    public class SMB2Client
    {
        ...
        public async Task<Tuple<SMB2FileStore, NTStatus>> TreeConnect(string shareName)
        {
            NTStatus status;
            ...
            if (response != null)
            {
                ...
                if (response.Header.Status == NTStatus.STATUS_SUCCESS && response is TreeConnectResponse)
                {
                    ...
                    return new Tuple<SMB2FileStore, NTStatus> (new SMB2FileStore(this, response.Header.TreeID, m_encryptSessionData || encryptShareData), status);
                }
            }
            else
            {
            ...
            return new Tuple<SMB2FileStore, NTStatus> (null, status);
        }
