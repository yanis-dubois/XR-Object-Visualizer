/*
 * This file was copied and modified from the SMBLibrary to make it asynchronous.
 * Here are the original copyrights terms :
 */
...
namespace SMBLibrary.Client
{
    public class SMB2AsyncClient
    {
        ...
        /// </param>
        public async Task<bool> Connect(string serverName, SMBTransportType transport)
        {
            bool is_connected = await Connect(serverName, transport, DefaultResponseTimeoutInMilliseconds);
            return is_connected;
        }

        public async Task<bool> Connect(string serverName, SMBTransportType transport, int responseTimeoutInMilliseconds)
        {
            ...
            bool is_connected = await Connect(serverAddress, transport, responseTimeoutInMilliseconds);
            return is_connected;
        }

        public async Task<bool> Connect(IPAddress serverAddress, SMBTransportType transport)
        {
            bool is_connected = await Connect(serverAddress, transport, DefaultResponseTimeoutInMilliseconds);
            return is_connected;
        }

        public async Task<bool> Connect(IPAddress serverAddress, SMBTransportType transport, int responseTimeoutInMilliseconds)
        {
            int port = (transport == SMBTransportType.DirectTCPTransport ? DirectTCPPort : NetBiosOverTCPPort);
            bool is_connected = await Connect(serverAddress, transport, port, responseTimeoutInMilliseconds);
            return is_connected;
        }

        internal async Task<bool> Connect(IPAddress serverAddress, SMBTransportType transport, int port, int responseTimeoutInMilliseconds)
        {
            ...
            if (!m_isConnected)
            {
                ...
                if (transport == SMBTransportType.NetBiosOverTCP)
                {
                    ...
                    SessionPacket sessionResponsePacket = await WaitForSessionResponsePacket();
                    if (!(sessionResponsePacket is PositiveSessionResponsePacket))
                    {
                        ...
                        sessionResponsePacket = await WaitForSessionResponsePacket();
                        if (!(sessionResponsePacket is PositiveSessionResponsePacket))
                        {
                            return false;
                        }
        ...
        private async Task<bool> NegotiateDialect()
        {
            ...
            NegotiateResponse response = await WaitForCommand(request.MessageID) as NegotiateResponse;
        ...
        public async Task<NTStatus> Login(string domainName, string userName, string password)
        {
            NTStatus status = await Login(domainName, userName, password, AuthenticationMethod.NTLMv2);
            return status;
        }
        ...
        public async Task<NTStatus> Login(string domainName, string userName, string password, AuthenticationMethod authenticationMethod)
        {
            ...
            NTStatus status = await Login(authenticationClient);
            return status;
        }

        public async Task<NTStatus> Login(IAuthenticationClient authenticationClient)
        {
            if (!m_isConnected)
            {
            ...
            TrySendCommand(request);
            SMB2Command response = await WaitForCommand(request.MessageID);
            if (response != null)
            {
        ...
        // public async Task<Tuple<List<string>, NTStatus>> ListShares()
        // {
        ...
        //     Tuple<ISMBFileStore, NTStatus> tuple = await TreeConnect("IPC$");
        //     ISMBFileStore namedPipeShare = tuple.Item1;
        ...
        //     return new Tuple<List<string>, NTStatus> (shares, status);
        // }

        public async Task<Tuple<SMB2AsyncFileStore, NTStatus>> TreeConnect(string shareName)
        {
            NTStatus status;
            ...
            if (response != null)
            {
                ...
                if (response.Header.Status == NTStatus.STATUS_SUCCESS && response is TreeConnectResponse)
                {
                    ...
                    return new Tuple<SMB2AsyncFileStore, NTStatus> (new SMB2AsyncFileStore(this, response.Header.TreeID, m_encryptSessionData || encryptShareData), status);
                }
            }
            else
            {
            ...
            return new Tuple<SMB2AsyncFileStore, NTStatus> (null, status);
        }
        ...
        internal async Task<SessionPacket> WaitForSessionResponsePacket()
        {
